...
if [[ $OSTYPE =~ ^darwin ]]; then
  sed -i .bak "s/mgfs_version = \".*\"$/mgfs_version = \"${new_version}\"/" "${DIR}/version.go"
  rm -f "${DIR}/version.go.bak"
else
  sed -i "s/mgfs_version = \".*\"$/mgfs_version = \"${new_version}\"/" "${DIR}/version.go"
fi
...
for i in $target_platforms; do
  binary_suffix=${i/-/_}
  echo "$binary_suffix"
  GOOS=$(echo $i | cut -d- -f1) GOARCH=$(echo $i | cut -d- -f2) go build -o "${DIR}/bin/mgfs-${new_version}-${binary_suffix}"
done
...
for i in $target_platforms; do
  binary_suffix=${i/-/_}
  echo "$binary_suffix"
  binary_sha256s="${binary_sha256s}mgfs-${new_version}-${binary_suffix}
  $(shasum -a 256 "${DIR}/bin/mgfs-${new_version}-${binary_suffix}" | awk '{print $1}')
...
github-release release --security-token "$github_token" --user CpuID --repo mgfs --tag "${new_version}" --name "mgfs ${new_version}" --description "${binary_sha256s}" --pre-release
...
for i in $target_platforms; do
  binary_suffix=${i/-/_}
  echo "$binary_suffix"
  zip "mgfs-${new_version}-${binary_suffix}.zip" "mgfs-${new_version}-${binary_suffix}"
done
cd "$DIR"
...
for i in $target_platforms; do
  ...
  sleep 1
  github-release upload --security-token "$github_token" --user CpuID --repo mgfs --tag "${new_version}" --name "mgfs-${new_version}-${binary_suffix}.zip" --file "${DIR}/bin/mgfs-${new_version}-${binary_suffix}.zip"
done
